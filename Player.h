...
*/
    ...
    Player(Deck deck, int N){
        //Need to get N cards in the hand object
        Hand tempHand(deck,N);
        hand = tempHand;
        // The problem is , is that the Hand is only in the player but it's not being displayed in the player 
    };
};
#endif